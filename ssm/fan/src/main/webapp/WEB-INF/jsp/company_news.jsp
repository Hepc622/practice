...
<body>
		...
		<div class="nyRight">
				...
				<div class="nyCont_con">
					...
					<div class="NewList">
						<c:if test="${not empty sessionScope.user }">
							<ul>
								<li>
									<dl>
										<dt>
											<a class="theme-login" href="javascript:;"><img
												src="${pageContext.request.contextPath}/images/add_normal_new.png"></a>
										</dt>
										<dd>
											<div class="txt"></div>
										</dd>
						...
						<c:forEach var="c" items="${ cons}">
							<ul>
								<li class="clickEvent" id="${c.context_id }"
									name="${sessionScope.user.user_id }">
									<dl>
										<dt>
											<a
												href="${pageContext.request.contextPath}/${nav.navigation_url }/single?id=${c.context_id }"
												title="右击删除"><img src="${c.context_img }"></a>
										</dt>
										<dd>
											<div class="titel">
												<a
													href="${pageContext.request.contextPath}/${nav.navigation_url }/single?id=${c.context_id }"
													title="">${c.context_title }</a> <span>[<fmt:formatDate
														value="${c.context_date }" dateStyle="long" />]
												</span>
											</div>
											<div class="txt">
												<c:choose>
													<c:when test="${fn:length(c.context_text) > 70}">
														<p>
															&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
															<c:out value="${fn:substring(c.context_text, 0, 70)}..." />
														</p>
													</c:when>
													<c:otherwise>
														<p>
															&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
															<c:out value="${c.context_text}..." />
														</p>
													</c:otherwise>
												</c:choose>
												<a
													href="${pageContext.request.contextPath}/${nav.navigation_url }/single?id=${c.context_id }"
													title="了解详情" class="detail">[点击详情]</a>
											</div>
										</dd>
									</dl>
								</li>
			...
			<div class="Pages">
				<span class="p_page"> <c:if test="${page.hasPrePage }">
						<a
							href="${pageContext.request.contextPath}/${nav.navigation_url }/${title.title_url }?page=${page.currentPage-1}&id=${title.title_id}"
							class="a_prev"></a>
					</c:if> <c:if test="${!page.hasPrePage }">
						<a href="javascript:void(0);" class="a_prev"></a>
					</c:if> <em class="num"><c:forEach var="i" begin="${page.startPage }"
							end="${page.endPage}">
							<c:if test="${page.currentPage ==i}">
								<a
									href="${pageContext.request.contextPath}/${nav.navigation_url }/${title.title_url }?page=${i }&id=${title.title_id}"
						...
						</c:forEach> </em> <c:if test="${!page.hasNextPage }">
	...
	<div class="clear"></div>
	<!-- 添加新闻页面 -->
	<div class="theme-popover-add-news">
		...
		<div class="theme-popbod dform">
			<form class="theme-signin" name="loginform"
				action="${pageContext.request.contextPath}/upload/news?id=${title.title_id }"
				method="post">
				<ol>
					<li><img class="show-img-news" id="showImg" src=""></li>
					<li></li>
					<li><strong>标题图片：</strong><input class="ipt" type="file"
						name="file" size="20" /></li>
					<li><strong>新闻标题：</strong><input class="ipt" type="text"
						name="title" size="20" /></li>
					<li><input name="ctext" type="text" style="display: none;" /></li>
				</ol>
				<br>编辑区：
				<div class="ContextEdite" contentEditable="true" name="ctext"></div>
				<input class="btn btn-primary" type="button" onclick="addContext();"
